...
export type getCriptoSymbolResponse = {
  data: Cripto;
  status: number;
  headers: Headers;
};

export const getGetCriptoSymbolUrl = (symbol: string) => {
  return `http://localhost:4003/cripto/${symbol}`;
};

export const getCriptoSymbol = async (
  symbol: string,
  options?: RequestInit
): Promise<getCriptoSymbolResponse> => {
  return fetcher<Promise<getCriptoSymbolResponse>>(
    getGetCriptoSymbolUrl(symbol),
    {
      ...options,
      method: "GET",
    }
...
export const getGetCriptoSymbolQueryKey = (symbol: string) => {
  return [`http://localhost:4003/cripto/${symbol}`] as const;
};

export const getGetCriptoSymbolQueryOptions = <
  TData = Awaited<ReturnType<typeof getCriptoSymbol>>,
  TError = GetCriptoSymbol404
>(
  symbol: string,
  options?: {
    query?: Partial<
      UseQueryOptions<
        Awaited<ReturnType<typeof getCriptoSymbol>>,
        TError,
        TData
      >
    >;
  ...
  const queryKey = queryOptions?.queryKey ?? getGetCriptoSymbolQueryKey(symbol);

  const queryFn: QueryFunction<Awaited<ReturnType<typeof getCriptoSymbol>>> = ({
    signal,
  }) => getCriptoSymbol(symbol, { signal, ...requestOptions });
...
export type GetCriptoSymbolQueryResult = NonNullable<
  Awaited<ReturnType<typeof getCriptoSymbol>>
>;
export type GetCriptoSymbolQueryError = GetCriptoSymbol404;

export function useGetCriptoSymbol<
  TData = Awaited<ReturnType<typeof getCriptoSymbol>>,
  TError = GetCriptoSymbol404
>(
  symbol: string,
...
>(
  ...
  const queryOptions = getGetCriptoSymbolQueryOptions(symbol, options);
